 ...
 *
 * Frequency Invariance support for RISC-V using the Sscpuutil extension
 * (CPU Utilization Counter Extension).
 *
 * The Sscpuutil extension provides a pair of CSR counters:
 *   CSR_SSCPUUTIL_CORECYC  - core-cycles counter, increments at the actual CPU
 *                          frequency
 *   CSR_SSCPUUTIL_ACTTIME  - actual-time counter, increments at a fixed
...
#undef pr_fmt
#define pr_fmt(fmt) "Sscpuutil: " fmt

static DEFINE_PER_CPU_READ_MOSTLY(unsigned long, arch_max_freq_scale);

/* Bitmask of CPUs for which Sscpuutil-based FIE has been successfully set up */
static cpumask_var_t sscpuutil_fie_cpus;
...
static inline bool freq_counters_valid(int cpu)
{
	...
	if (!__riscv_isa_extension_available(NULL, RISCV_ISA_EXT_SSCPUUTIL)) {
		pr_debug("CPU%d: Sscpuutil counters are not supported.\n", cpu);
		return false;
	}
...
static u64 sscpuutil_get_max_rate(int cpu)
{
	u64 rate = 0;
...
/**
 * sscpuutil_set_max_ratio - pre-compute the ref/max frequency scale factor
 * @cpu:      logical CPU number
 * @max_rate: maximum CPU frequency in Hz
 * @ref_rate: fixed frequency of the Sscpuutil reference counter in Hz
 *
 ...
 * underflows to zero.
 */
static int sscpuutil_set_max_ratio(int cpu, u64 max_rate, u64 ref_rate)
{
	u64 ratio;
	...
	if (!ratio) {
		WARN_ONCE(1, "Sscpuutil reference frequency too low.\n");
		return -EINVAL;
	}
...
/**
 * sscpuutil_scale_freq_tick - update arch_freq_scale on every scheduler tick
 *
 * Registered as the set_freq_scale callback of sscpuutil_sfd and invoked by
 ...
 */
static void sscpuutil_scale_freq_tick(void)
{
...
static struct scale_freq_data sscpuutil_sfd = {
	.source        = SCALE_FREQ_SOURCE_ARCH,
	.set_freq_scale = sscpuutil_scale_freq_tick,
};

/**
 * sscpuutil_fie_setup - enable Sscpuutil-based FIE for a set of CPUs
 * @cpus: the set of CPUs to enable FIE for
 *
 * For each CPU in @cpus, validates counter availability and computes the
 * max-frequency ratio.  If all CPUs pass, registers sscpuutil_sfd as the
 * SCALE_FREQ_SOURCE_ARCH source for those CPUs.
 *
 * The function is idempotent: CPUs already in sscpuutil_fie_cpus are skipped.
 */
static void sscpuutil_fie_setup(const struct cpumask *cpus)
{
	int cpu;

	/* Nothing to do if all CPUs in this policy are already set up */
	if (unlikely(cpumask_subset(cpus, sscpuutil_fie_cpus)))
		return;
	...
	for_each_cpu(cpu, cpus) {
		...
		u64 max_rate = sscpuutil_get_max_rate(cpu);
		...
		if (sscpuutil_set_max_ratio(cpu, max_rate, riscv_timebase)) {
			pr_info("CPU%d: sscpuutil_set_max_ratio() failed, skip FIE setup\n", cpu);
			return;
		}
	}

	cpumask_or(sscpuutil_fie_cpus, sscpuutil_fie_cpus, cpus);

	topology_set_scale_freq_source(&sscpuutil_sfd, sscpuutil_fie_cpus);

	pr_info("CPUs[%*pbl]: Sscpuutil FIE enabled\n",
		cpumask_pr_args(cpus));
}

/**
 * init_sscpuutil_fie - initialise Sscpuutil-based FIE for all present CPUs
 *
 ...
 */
static int __init init_sscpuutil_fie(void)
{
	if (!zalloc_cpumask_var(&sscpuutil_fie_cpus, GFP_KERNEL))
		return -ENOMEM;

	sscpuutil_fie_setup(cpu_present_mask);
